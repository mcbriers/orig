...
class ImportExport:
    ...
    @staticmethod
    def import_rfid_csv(file_path: str):
        """Import an RFID/Transponder CSV file.

        Accepts files with or without a header line. Expected columns (in order):
        Id, Description, MagnetOnly, TransponderId

        Returns a list of dicts with keys: 'id' (int), 'description' (str),
        'magnet_only' (bool), 'transponder_id' (str).
        """
        results = []
        try:
            with open(file_path, 'r', newline='') as f:
                ...
                lower_first = [c.lower() for c in first]
                has_header = False
                # detect header by presence of id and transponder keywords
                if ('id' in lower_first and ('transponderid' in ''.join(lower_first) or 'transponderid' in lower_first or 'transponder id' in ' '.join(lower_first))):
                    has_header = True
                ...
                for r in data_rows:
                    ...
                    while len(r) < 4:
                        r.append('')
                    raw_id, raw_desc, raw_magnet, raw_trans = (c.strip() for c in r[:4])
                    # parse id
                    try:
                        pid = int(raw_id)
                    except Exception:
                        # If id is empty or non-numeric, skip row
                        continue
                    desc = raw_desc
                    # parse magnet-only as boolean
                    mlow = raw_magnet.lower()
                    magnet = True if mlow in ('1', 'true', 'yes', 'y', 't') else False
                    trans_id = raw_trans
                    results.append({
                        'id': pid,
                        'description': desc,
            ...
            print(f"Failed to import RFID CSV: {e}")
            return results
