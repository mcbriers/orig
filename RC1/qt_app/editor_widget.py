...
class PointsTableModel(QAbstractTableModel):
    ...
    def flags(self, index: QModelIndex):
        """Return item flags - make all columns editable except ID and Refs."""
        if not index.isValid():
            return Qt.ItemFlag.NoItemFlags
        f = Qt.ItemFlag.ItemIsEnabled | Qt.ItemFlag.ItemIsSelectable
        col = index.column()
        # Make all columns editable except ID (0) and Refs (5)
        if col not in [0, 5]:
            f |= Qt.ItemFlag.ItemIsEditable
        return f
    
    def setData(self, index: QModelIndex, value, role=Qt.ItemDataRole.EditRole):
        """Handle editing of point data."""
        if not index.isValid() or not self.project or role != Qt.ItemDataRole.EditRole:
            return False
        ...
        try:
            if col == 0:  # ID - not editable (would break references)
                return False
            elif col == 1:  # Real X
                point.real_x = int(value)
            elif col == 2:  # Real Y
                point.real_y = int(value)
            elif col == 3:  # Z
                point.z = int(value)
            elif col == 4:  # Hidden
                point.hidden = str(value).lower() in ('yes', 'true', '1')
            elif col == 5:  # Refs - not editable
                return False
            elif col == 6:  # Description
                point.description = str(value)
            else:
                return False
...
class LinesTableModel(QAbstractTableModel):
    ...
    def flags(self, index: QModelIndex):
        """Return item flags - make relevant columns editable."""
        if not index.isValid():
            return Qt.ItemFlag.NoItemFlags
        f = Qt.ItemFlag.ItemIsEnabled | Qt.ItemFlag.ItemIsSelectable
        col = index.column()
        # Editable: Start ID, End ID, Hidden, Description
        # Not editable: ID (0), Z (3 - calculated), Base for Curve (5 - calculated)
        if col in [1, 2, 4, 6]:
            f |= Qt.ItemFlag.ItemIsEditable
        return f
    
    def setData(self, index: QModelIndex, value, role=Qt.ItemDataRole.EditRole):
        """Handle editing of line data."""
        if not index.isValid() or not self.project or role != Qt.ItemDataRole.EditRole:
            return False
        ...
        try:
            if col == 0:  # ID - not editable
                return False
            elif col == 1:  # Start ID
                new_id = int(value)
                new_point = self.project.get_point(new_id)
                if not new_point:
                    return False
                # Prevent zero-length lines
                if new_id == line.end_id:
                    return False
                line.start_id = new_id
            elif col == 2:  # End ID
                new_id = int(value)
                new_point = self.project.get_point(new_id)
                if not new_point:
                    return False
                # Prevent zero-length lines
                if new_id == line.start_id:
                    return False
                line.end_id = new_id
            elif col == 3:  # Z - not editable (calculated)
                return False
            elif col == 4:  # Hidden
                line.hidden = str(value).lower() in ('yes', 'true', '1')
            elif col == 5:  # Base for Curve - not editable (calculated)
                return False
            elif col == 6:  # Description
...
class CurvesTableModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        arc_samples = self._get_arc_samples(curve)
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            if col == 0:  # ID
                ...
                pos = col - 5
                if arc_samples and pos < len(arc_samples):
                    x, y, z = arc_samples[pos]
        ...
        if role == Qt.ItemDataRole.ToolTipRole and col >= 5:
            ...
            if arc_samples and pos < len(arc_samples):
                x, y, z = arc_samples[pos]
                return f"Real coordinates: ({int(x)}, {int(y)}, {int(z)})"
            if self.project and curve.arc_point_ids and pos < len(curve.arc_point_ids):
                point_id = curve.arc_point_ids[pos]
                point = self.project.get_point(point_id)
                if point:
                    return (
                        f"Point {point_id}: real=({int(point.real_x)}, {int(point.real_y)}, {int(point.z)}) "
                        f"pdf=({round(point.pdf_x, 3)}, {round(point.pdf_y, 3)})"
                    )
    ...
    def _get_sorted_curves(self):
        ...
        def sort_key(curve):
            if self._sort_column == 0:  # ID
                ...
                samples = self._get_arc_samples(curve)
    ...
    def _get_arc_samples(self, curve) -> List[tuple]:
        """Return interior arc sample coordinates for display."""
        if curve.arc_points_real and len(curve.arc_points_real) > 2:
            return curve.arc_points_real[1:-1]
        return []

    def _update_max_positions(self):
        """Update the number of arc position columns based on available data."""
        curves_list = self.project.curves if self.project else self.curves
        max_count = 0
        for curve in curves_list:
            ...
            elif curve.arc_point_ids:
                max_count = max(max_count, len(curve.arc_point_ids))
        # Keep a minimum number of columns for consistent layout
        self.max_positions = max(6, max_count)
    ...
    def flags(self, index: QModelIndex):
        ...
        col = index.column()
        # Editable: Start ID, End ID, Hidden, Description
        # Not editable: ID (0), Arc point positions (read-only display)
        if col in [1, 2, 3, 4]:
            f |= Qt.ItemFlag.ItemIsEditable
        return f
    
    def setData(self, index: QModelIndex, value, role=Qt.ItemDataRole.EditRole):
        """Handle editing of curve data."""
        if not index.isValid() or not self.project or role != Qt.ItemDataRole.EditRole:
            return False
        ...
        try:
            if col == 0:  # ID - not editable
                ...
                if not new_point:
                    return False
                # Prevent degenerate curves
                if new_id == curve.end_id:
                    return False
                curve.start_id = new_id
            elif col == 2:  # End ID
                new_id = int(value)
                new_point = self.project.get_point(new_id)
                if not new_point:
                    return False
                # Prevent degenerate curves
                if new_id == curve.start_id:
                    return False
                curve.end_id = new_id
            elif col == 3:  # Hidden
                curve.hidden = str(value).lower() in ('yes', 'true', '1')
            elif col == 4:  # Description
                curve.description = str(value)
            elif col >= 5:  # Arc point positions - not editable
                return False
            else:
                return False
...
class TransponderTableModel(QAbstractTableModel):
    """Table model for RFID / transponder records stored in project.transponders."""

    def __init__(self, project=None):
        super().__init__()
        self.project = project
        # Add 'Line ID' and 'Distance' columns to show assigned line info
        self.headers = ['Id', 'Description', 'MagnetOnly', 'TransponderId', 'Line ID', 'Distance', 'Assigned']

    def rowCount(self, parent=QModelIndex()):
        if self.project:
            return len(self.project.transponders or [])
        return 0
    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        rec: Dict[str, Any] = rows[index.row()]
        col = index.column()

        # Ensure default value for assigned
        if 'assigned' not in rec:
            rec['assigned'] = False
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            if col == 0:
                ...
                return 'Yes' if rec.get('magnet_only') else 'No'
            elif col == 3:
                return rec.get('transponder_id', '')
            elif col == 4:
                # Line ID (assigned line) - show empty if not assigned
                lid = rec.get('line_id')
                return str(lid) if lid is not None and lid != '' else ''
            elif col == 5:
                # Distance - show empty if not assigned
                dist = rec.get('distance')
                return str(dist) if dist is not None and dist != '' else ''
            elif col == 6:
                return 'Yes' if rec.get('assigned') else 'No'

        # Provide checkbox state for 'Assigned' column (now col index 6)
        if role == Qt.ItemDataRole.CheckStateRole and col == 6:
            return Qt.CheckState.Checked if rec.get('assigned') else Qt.CheckState.Unchecked
        
        # Provide checkbox state for 'MagnetOnly' column (col index 2)
        if role == Qt.ItemDataRole.CheckStateRole and col == 2:
            return Qt.CheckState.Checked if rec.get('magnet_only') else Qt.CheckState.Unchecked
    ...
    def flags(self, index: QModelIndex):
        ...
        # Make most columns editable including Id
        if col in [0, 1, 3, 4, 5]:  # Id, Description, TransponderId, Line ID, Distance
            f |= Qt.ItemFlag.ItemIsEditable
        # Make 'MagnetOnly' and 'Assigned' columns checkable
        elif col in [2, 6]:  # MagnetOnly, Assigned
            f |= Qt.ItemFlag.ItemIsUserCheckable | Qt.ItemFlag.ItemIsEditable
        
        return f

    def setData(self, index: QModelIndex, value, role=Qt.ItemDataRole.EditRole):
        if not index.isValid() or not self.project:
            return False
        rows = self.project.transponders or []
        if not (0 <= index.row() < len(rows)):
            return False
        ...
        if role == Qt.ItemDataRole.CheckStateRole:
            if col == 2:  # MagnetOnly
                rec['magnet_only'] = (value == Qt.CheckState.Checked)
                self.project.modified = True
                self.dataChanged.emit(index, index, [role])
                return True
            elif col == 6:  # Assigned
                is_assigned = (value == Qt.CheckState.Checked)
                rec['assigned'] = is_assigned
                # If unchecking (unassigning), clear the line_id and distance
                if not is_assigned:
                    rec['line_id'] = None
                    rec['distance'] = None
                    # Update adjacent columns to reflect the changes
                    line_id_index = self.index(index.row(), 4)
                    distance_index = self.index(index.row(), 5)
                    self.dataChanged.emit(line_id_index, distance_index, [Qt.ItemDataRole.DisplayRole])
                self.project.modified = True
                self.dataChanged.emit(index, index, [role])
                return True
        ...
        if role == Qt.ItemDataRole.EditRole:
            if col == 0:  # Id
                try:
                    rec['id'] = int(value) if value else None
                    self.project.modified = True
                    self.dataChanged.emit(index, index, [role])
                    return True
                except (ValueError, TypeError):
                ...
                rec['description'] = str(value)
                self.project.modified = True
                self.dataChanged.emit(index, index, [role])
                return True
            elif col == 3:  # TransponderId
                rec['transponder_id'] = str(value)
                self.project.modified = True
                self.dataChanged.emit(index, index, [role])
                return True
            elif col == 4:  # Line ID
                try:
                    rec['line_id'] = int(value) if value else None
                    self.project.modified = True
                    self.dataChanged.emit(index, index, [role])
                    return True
                except (ValueError, TypeError):
                ...
                try:
                    rec['distance'] = int(value) if value else None
                    self.project.modified = True
                    self.dataChanged.emit(index, index, [role])
                    return True
                except (ValueError, TypeError):
    ...
    def sort(self, column: int, order: Qt.SortOrder = Qt.SortOrder.AscendingOrder):
        """Sort transponders list in-place by given column."""
        if not self.project:
            return
        keymap = {
            0: lambda r: int(r.get('id', 0)),
            1: lambda r: (r.get('description') or '').lower(),
            2: lambda r: bool(r.get('magnet_only')),
            3: lambda r: (r.get('transponder_id') or ''),
            4: lambda r: int(r.get('line_id') or 0),
            5: lambda r: int(r.get('distance') or 0),
            6: lambda r: bool(r.get('assigned')),
        }
        key = keymap.get(column, lambda r: 0)
        reverse = (order == Qt.SortOrder.DescendingOrder)
        try:
            self.layoutAboutToBeChanged.emit()
            self.project.transponders.sort(key=key, reverse=reverse)
            self.layoutChanged.emit()
        except Exception:
            pass
...
class EditorWidget(QWidget):
    ...
    # Request to assign a transponder (row index in project.transponders)
    assign_transponder_requested = pyqtSignal(int)
    # Request to unassign a transponder (row index in project.transponders)
    unassign_transponder_requested = pyqtSignal(int)
    # Signal when data changes that requires view refresh
    ...
    def _setup_ui(self):
        ...
        # Refresh tables when switching tabs so the visible table is up-to-date
        try:
        ...
        rfid_widget.setLayout(rfid_layout)

        self.rfid_table = QTableView()
        self.rfid_model = TransponderTableModel(project=self.project)
        self.rfid_table.setModel(self.rfid_model)
        self.rfid_table.setSelectionBehavior(QTableView.SelectionBehavior.SelectRows)
        self.rfid_table.setSelectionMode(QTableView.SelectionMode.SingleSelection)
        self.rfid_table.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.rfid_table.customContextMenuRequested.connect(self._show_rfid_context_menu)
        self.rfid_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.ResizeToContents)
        self.rfid_table.setSortingEnabled(True)
        # Connect dataChanged signal to trigger view refresh
        self.rfid_model.dataChanged.connect(lambda: self.data_changed.emit())
        rfid_layout.addWidget(self.rfid_table)

        self.tabs.addTab(rfid_widget, "RFID")
    ...
    def refresh(self):
        ...
        # Refresh RFID model
        try:
            if hasattr(self, 'rfid_model') and self.rfid_model:
                self.rfid_model.beginResetModel()
                self.rfid_model.endResetModel()
        except Exception:
            pass
    ...
    def _show_point_context_menu(self, position):
        ...
        # Obtain IDs from the model (respect current sort order)
        model = self.points_table.model()
        point_ids = []
        selected_points = []
        for index in selected_indexes:
            id_val = model.data(model.index(index.row(), 0), Qt.ItemDataRole.DisplayRole)
            try:
                pid = int(id_val)
            except Exception:
                continue
    ...
    def _show_line_context_menu(self, position):
        ...
        for index in selected_indexes:
            ...
            try:
                ...
                continue
            line = self.project.get_line(lid)
            if line:
    ...
    def _show_curve_context_menu(self, position):
        ...
        for index in selected_indexes:
            ...
            try:
                ...
                continue
            curve = self.project.get_curve(cid)
            if curve:
    ...
    def _show_rfid_context_menu(self, position):
        """Show context menu for RFID table."""
        selected = self.rfid_table.selectionModel().selectedRows()
        if not selected:
            return
        ...
        # Check if this transponder is assigned
        transponder = self.project.transponders[row] if row < len(self.project.transponders) else None
        is_assigned = transponder and transponder.get('assigned', False)

        menu = QMenu()
        assign_action = QAction("Assign to Line...", self)
        assign_action.triggered.connect(lambda: self.assign_transponder_requested.emit(row))
        menu.addAction(assign_action)
        
        unassign_action = QAction("Unassign", self)
        unassign_action.setEnabled(is_assigned)
        unassign_action.triggered.connect(lambda: self.unassign_transponder_requested.emit(row))
        menu.addAction(unassign_action)

        menu.exec(self.rfid_table.viewport().mapToGlobal(position))
