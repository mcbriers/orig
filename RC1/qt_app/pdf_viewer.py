...
class PDFViewer(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.curve_polylines = []  # [(curve_id, baseline_points, sample_points), ...]
    ...
    def _draw_overlays(self):
        ...
        for curve_data in self.curve_polylines:
            if len(curve_data) >= 2:
                curve_id, baseline, samples = curve_data

                # Draw baseline
                if len(baseline) >= 2:
                    if curve_id in self.highlighted_curves:
                        pen = QPen(QColor(255, 165, 0), self.curve_width * 3)
                    else:
                ...
                # Draw intermediate sample points as small circles
                if samples:
                    if curve_id in self.highlighted_curves:
                        pen = QPen(QColor(255, 165, 0), 4)
                        radius = self.point_size * 1.5
                    else:
                    ...
                    for sx, sy in samples:
                        painter.drawEllipse(
                            QPointF(sx * self.zoom_level, sy * self.zoom_level),
                            radius, radius
                        )
    ...
    def _find_curve_at(self, pdf_x: float, pdf_y: float, tolerance: float = 8.0) -> int:
        ...
        for curve_data in self.curve_polylines:
            ...
            if min_dist <= tolerance_scaled and min_dist < closest_dist:
                ...
                # Also allow picking directly on sample points (treat as circles with tolerance)
                for sx, sy in samples:
                    dist = ((pdf_x - sx) ** 2 + (pdf_y - sy) ** 2) ** 0.5
                    if dist <= tolerance_scaled and dist < closest_dist:
                        closest_id = curve_id
                        closest_dist = dist
        return closest_id
    ...
    def set_curves(self, curves_data):
        """Set curve baseline and sample points.

        curves_data format: [(curve_id, baseline_points, sample_points), ...]
        where baseline_points is a list of at least start/end PDF coords and
        sample_points is a list of interior PDF coords along the arc.
        """
