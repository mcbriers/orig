...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Remember last RFID distance input (integer)
        self.last_rfid_distance = 100
        # History of recent RFID assignments: list of (id_str_or_int, line_id, timestamp)
        self.rfid_assign_history = []
    ...
    def _setup_ui(self):
        ...
        # Connect data changes to refresh views
        self.editor.data_changed.connect(self._refresh_all_views)
        # Connect RFID assignment from editor (right-click in RFID tab)
        try:
            self.editor.assign_transponder_requested.connect(self._assign_rfid_from_rfid_tab)
            self.editor.unassign_transponder_requested.connect(self._unassign_rfid_from_rfid_tab)
        except Exception:
            pass
        ...
        try:
            ...
            self.viewer_3d.rfid_unassign_requested.connect(self._unassign_rfid_from_3d)
            self.viewer_3d.rfid_edit_requested.connect(self._edit_rfid_from_3d)
        except Exception:
            pass
    ...
    def _assign_rfid_to_line(self, line_id: int):
        """Open dialog to assign an unassigned transponder to a given line (from 3D view)."""
        from PyQt6.QtWidgets import QDialog, QVBoxLayout, QLabel, QLineEdit, QSpinBox, QDialogButtonBox, QCompleter
        from PyQt6.QtCore import QStringListModel

        # Build list of unassigned transponder Ids (numeric Id column strings)
        candidates = []
        for rec in (self.project.transponders or []):
            ...
            try:
                ...
                if cid is None:
                    continue
                candidates.append(str(int(cid)))
            except Exception:
                # fallback to string representation
                candidates.append(str(rec.get('id', '')))

        dialog = QDialog(self)
        dialog.setWindowTitle(f"Assign RFID to Line {line_id}")
        layout = QVBoxLayout(dialog)

        layout.addWidget(QLabel("Search RFID (type digits or paste full id):"))
        search_input = QLineEdit()
        layout.addWidget(search_input)

        # Prediction: try to prefill from recent history
        pred_list = self.predict_next_rfid()
        pred_label = None
        if pred_list:
            top_pred, conf, kind = pred_list[0][:3]
            try:
                pred_label = QLabel(f"Predicted next Id: {top_pred} (conf {int(conf*100)}%)")
                pred_label.setStyleSheet('color: #444; font-size: 11px')
                layout.addWidget(pred_label)
                # Prefill and select text for quick overwrite
                search_input.setText(str(top_pred))
                search_input.selectAll()
            except Exception:
                pass

        # Distance input (integer)
        layout.addWidget(QLabel("Distance (integer):"))
        dist_spin = QSpinBox()
        dist_spin.setRange(0, 1000000)
        dist_spin.setValue(int(self.last_rfid_distance or 0))
        layout.addWidget(dist_spin)

        # Completer with available candidates
        try:
            completer = QCompleter()
            model = QStringListModel(candidates)
        ...
        key = search_input.text().strip()
        if not key:
            QMessageBox.warning(self, "No RFID Selected", "Please enter or select an RFID to assign.")
            return

        # Find matching unassigned record by numeric Id (Id column)
        matched = None
        for rec in (self.project.transponders or []):
            if rec.get('assigned'):
                continue
            try:
                if int(key) == int(rec.get('id')):
                    matched = rec
                    break
            except Exception:
                # if key is non-numeric, try matching transponder_id as fallback
                if rec.get('transponder_id') and rec.get('transponder_id') == key:
                    matched = rec
                    break

        if not matched:
            # RFID not found - offer to add it
            from PyQt6.QtWidgets import QMessageBox, QCheckBox
            
            msg_box = QMessageBox(self)
            msg_box.setWindowTitle("RFID Not Found")
            msg_box.setText(f"Transponder '{key}' not found in the list.")
            msg_box.setInformativeText("Would you like to add it to the project?")
            
            # Add checkbox for magnet-only option
            magnet_checkbox = QCheckBox("Magnet only RFID")
            magnet_checkbox.setChecked(True)  # Default to magnet-only
            msg_box.setCheckBox(magnet_checkbox)
            
            add_button = msg_box.addButton("Add and Assign", QMessageBox.ButtonRole.AcceptRole)
            ...
            # Create new transponder record
            is_magnet_only = magnet_checkbox.isChecked()
            
            # Use entered value as the id (database key)
            # transponder_id left empty for manual entry
            try:
                new_id = int(key)
            except (ValueError, TypeError):
                QMessageBox.warning(self, "Invalid ID", f"'{key}' is not a valid numeric ID.")
                return
            
            # Prompt for transponder ID
            from PyQt6.QtWidgets import QInputDialog
            transponder_id, ok = QInputDialog.getText(
                self,
                "Enter Transponder ID",
                f"New RFID {new_id} created.\nEnter Transponder ID (can be left blank):",
            ...
            new_transponder = {
                'id': new_id,
                'transponder_id': transponder_id.strip(),
                'description': '',
                'magnet_only': is_magnet_only,
                'assigned': False,
                'line_id': None,
                'distance': None
            }
            
            if self.project.transponders is None:
                self.project.transponders = []
            self.project.transponders.append(new_transponder)
            
            matched = new_transponder
            self.update_status(f"Added new {'magnet-only ' if is_magnet_only else ''}RFID '{key}' to project")

        # Assign
        matched['assigned'] = True
        matched['line_id'] = int(line_id)
        matched['distance'] = int(dist_spin.value())
        self.last_rfid_distance = int(dist_spin.value())
        self.project.modified = True
        # Record to history
        try:
            # store numeric id when possible
            store_id = matched.get('id')
            try:
                store_id = int(store_id)
            except Exception:
                store_id = str(matched.get('id'))
            self.rfid_assign_history.append((store_id, int(line_id), time.time()))
            # cap history size
            if len(self.rfid_assign_history) > 50:
                self.rfid_assign_history = self.rfid_assign_history[-50:]
        except Exception:
            pass
        # Refresh UI
        self._refresh_all_views()
        self.update_status(f"Assigned RFID '{matched.get('transponder_id') or matched.get('id')}' to Line {line_id}")

    def _assign_rfid_from_rfid_tab(self, row_index: int):
        """Assign a transponder selected in the RFID table to a line (from Editor tab)."""
        from PyQt6.QtWidgets import QDialog, QVBoxLayout, QLabel, QComboBox, QSpinBox, QDialogButtonBox

        rows = self.project.transponders or []
        if not (0 <= row_index < len(rows)):
            QMessageBox.warning(self, "Selection Error", "Selected transponder not found.")
            return

        rec = rows[row_index]

        dialog = QDialog(self)
        dialog.setWindowTitle(f"Assign Transponder {rec.get('transponder_id') or rec.get('id')}")
        layout = QVBoxLayout(dialog)

        layout.addWidget(QLabel("Select Line to assign:"))
        combo = QComboBox()
        # Populate with available lines
        for line in self.project.lines:
            combo.addItem(f"{line.id}: {line.start_id}→{line.end_id}", userData=line.id)
        ...
        if line_id is None:
            QMessageBox.warning(self, "No Line", "Please select a line to assign.")
            return
        rec['assigned'] = True
        rec['line_id'] = int(line_id)
        rec['distance'] = int(dist_spin.value())
        self.last_rfid_distance = int(dist_spin.value())
        self.project.modified = True
        # Record to history
        try:
        ...
        self._refresh_all_views()
        self.update_status(f"Assigned transponder '{rec.get('transponder_id') or rec.get('id')}' to Line {line_id}")

    def _unassign_rfid_from_rfid_tab(self, row_index: int):
        """Unassign a transponder selected in the RFID table (from Editor tab)."""
        rows = self.project.transponders or []
        if not (0 <= row_index < len(rows)):
            QMessageBox.warning(self, "Selection Error", "Selected transponder not found.")
            return
        ...
        if not rec.get('assigned', False):
            QMessageBox.information(self, "Not Assigned", "This transponder is not currently assigned to any line.")
            return
        ...
        self._refresh_all_views()
        self.update_status(f"Unassigned transponder '{rec.get('transponder_id') or rec.get('id')}'")

    def _unassign_rfid_from_3d(self, transponder_id: int):
        """Unassign a transponder clicked in the 3D viewer."""
        # Find the transponder by ID
        transponder = None
        for rec in self.project.transponders:
            if rec.get('id') == transponder_id or rec.get('transponder_id') == transponder_id:
                transponder = rec
                break
        
        if not transponder:
            QMessageBox.warning(self, "Not Found", "Transponder not found in project.")
            return
        
        # Check if currently assigned
        if not transponder.get('assigned', False):
            QMessageBox.information(self, "Not Assigned", "This transponder is not currently assigned to any line.")
            return
        
        # Clear assignment
        transponder['assigned'] = False
        transponder['line_id'] = None
        transponder['distance'] = None
        self.project.modified = True
        
        self._refresh_all_views()
        self.update_status(f"Unassigned transponder '{transponder.get('transponder_id') or transponder.get('id')}'")

    def _edit_rfid_from_3d(self, transponder_id: int):
        """Edit an RFID from the 3D viewer."""
        from PyQt6.QtWidgets import QDialog, QVBoxLayout, QLabel, QLineEdit, QSpinBox, QCheckBox, QDialogButtonBox
        
        # Find the transponder by ID
        transponder = None
        for rec in self.project.transponders:
            if rec.get('id') == transponder_id:
                transponder = rec
                break
        ...
        # Create edit dialog
        dialog = QDialog(self)
        dialog.setWindowTitle(f"Edit RFID {transponder.get('id')}")
        layout = QVBoxLayout(dialog)
        
        # ID field
        layout.addWidget(QLabel("ID (Key):"))
        id_input = QLineEdit()
        id_input.setText(str(transponder.get('id', '')))
        layout.addWidget(id_input)
        
        # Transponder ID field
        layout.addWidget(QLabel("Transponder ID:"))
        transponder_id_input = QLineEdit()
        transponder_id_input.setText(str(transponder.get('transponder_id', '')))
        layout.addWidget(transponder_id_input)
        
        # Description field
        layout.addWidget(QLabel("Description:"))
        description_input = QLineEdit()
        description_input.setText(str(transponder.get('description', '')))
        layout.addWidget(description_input)
        
        # Magnet Only checkbox
        magnet_checkbox = QCheckBox("Magnet Only")
        magnet_checkbox.setChecked(transponder.get('magnet_only', False))
        layout.addWidget(magnet_checkbox)
        
        # Line ID (if assigned)
        layout.addWidget(QLabel("Line ID (blank to keep current):"))
        line_id_input = QLineEdit()
        if transponder.get('line_id'):
            line_id_input.setText(str(transponder.get('line_id')))
        layout.addWidget(line_id_input)
        
        # Distance field
        layout.addWidget(QLabel("Distance:"))
        distance_spin = QSpinBox()
        distance_spin.setRange(0, 1000000)
        distance_spin.setValue(int(transponder.get('distance', 0)))
        ...
        # Update transponder with new values
        try:
            new_id = int(id_input.text().strip())
            transponder['id'] = new_id
        except (ValueError, TypeError):
            QMessageBox.warning(self, "Invalid ID", "ID must be a valid integer.")
            return
        
        transponder['transponder_id'] = transponder_id_input.text().strip()
        transponder['description'] = description_input.text().strip()
        transponder['magnet_only'] = magnet_checkbox.isChecked()
        
        # Update line_id if provided
        line_id_text = line_id_input.text().strip()
        if line_id_text:
            try:
                transponder['line_id'] = int(line_id_text)
                transponder['assigned'] = True
            except (ValueError, TypeError):
                pass
        
        transponder['distance'] = distance_spin.value()
        
        self.project.modified = True
        self._refresh_all_views()
        self.update_status(f"Updated RFID {new_id}")

    def predict_next_rfid(self, lookback: int = 6):
        """Predict next RFID id(s) from recent assignment history.

        Returns list of tuples: (candidate_str, confidence_float, kind, step_int)
        kind is 'numeric' or 'suffix' or 'none'.
        """
        hist = list(self.rfid_assign_history or [])
        if not hist:
            return []

        # Use last `lookback` numeric-like ids
        entries = hist[-lookback:]
        ...
        for item in entries:
            ...
            if m:
                ...
                width = len(m.group(2))
                suffix_groups.setdefault((prefix, width), []).append(digits)

        # Numeric prediction
        if len(nums) >= 2:
            deltas = [nums[i] - nums[i-1] for i in range(1, len(nums))]
            # choose mode if available else median
            try:
                from collections import Counter
                cnt = Counter(deltas)
                step, cntv = cnt.most_common(1)[0]
                conf = cntv / len(deltas)
            except Exception:
                # fallback median
                sd = sorted(deltas)
                step = sd[len(sd)//2]
            ...
            return [(str(candidate), float(conf), 'numeric', int(step))]

        # Suffix-style prediction (same prefix/width)
        best = None
        for (prefix, width), digits_list in suffix_groups.items():
            if len(digits_list) >= 2:
                deltas = [digits_list[i] - digits_list[i-1] for i in range(1, len(digits_list))]
                try:
                    from collections import Counter
                    cnt = Counter(deltas)
                    step, cntv = cnt.most_common(1)[0]
                    ...
                    step = sd[len(sd)//2]
                    conf = 0.5
                candidate_num = digits_list[-1] + step
                candidate_str = f"{prefix}{str(candidate_num).zfill(width)}"
                best = (candidate_str, float(conf), 'suffix', int(step))
                break
    ...
    def _create_menus(self):
        ...
        export_rfid_sql_action = QAction("RFID SQL Update Script...", self)
        export_rfid_sql_action.triggered.connect(self.export_rfid_sql)
        export_menu.addAction(export_rfid_sql_action)

        # Import RFID/Transponder CSV
        import_rfid_action = QAction("Import RFID CSV...", self)
        import_rfid_action.triggered.connect(self.import_rfid_csv)
        file_menu.addAction(import_rfid_action)
    ...
    def export_rfid_sql(self):
        """Export RFID data as SQL UPDATE statements."""
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Export RFID SQL Update Script", "", "SQL Files (*.sql);;All Files (*)"
        )
        if not file_path:
            return
        ...
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write("-- RFID Assignment UPDATE Script\n")
                f.write("-- Generated by 3DMaker Digitizer\n\n")
                
                # Only export assigned RFIDs
                assigned_count = 0
                for rec in (self.project.transponders or []):
                    ...
                    # Generate UPDATE statement using id (the Vertex Id) as the key
                    # Convert distance to int for SQL
                    distance_int = int(distance) if distance is not None else 0
                    sql = f"UPDATE SeasPathDB.dbo.Rfid\n"
                    sql += f"SET VisualizationEdge = {line_id}, Length = {distance_int}\n"
                    sql += f"WHERE Id = (SELECT RfidId FROM SeasPathDB.dbo.Vertex WHERE Id = {rfid_id});\n\n"
                    
                    f.write(sql)
                    assigned_count += 1
                
                if assigned_count == 0:
                    f.write("-- No assigned RFIDs to export\n")
            
            self.update_status(f"RFID SQL script exported ({assigned_count} records)")
            QMessageBox.information(self, "Export Complete", f"Exported {assigned_count} RFID UPDATE statements")
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to export RFID SQL script: {e}")

    def import_rfid_csv(self):
        """Import an RFID/Transponder CSV and store in project.transponders."""
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Import RFID CSV", "", "CSV Files (*.csv);;All Files (*)"
        )
        if not file_path:
            return

        rows = self.import_export.import_rfid_csv(file_path)
        if not rows:
            QMessageBox.information(self, "Import RFID", "No valid records found in the selected file.")
            return

        # Store parsed rows in project and mark modified
        try:
            self.project.transponders = rows
            self.project.modified = True
            QMessageBox.information(self, "Import RFID", f"Imported {len(rows)} transponder record(s).")
            self.update_status(f"Imported {len(rows)} transponder record(s)")
        except Exception as e:
            QMessageBox.critical(self, "Import Failed", f"Failed to import RFID CSV: {e}")
...
<tr><td><b>Scroll wheel</b></td><td>Zoom in/out</td></tr>
    ...
    def show_settings(self):
        ...
        # 3D RFID settings (color and cube size)
        view3d_rfid_row = QHBoxLayout()
        view3d_rfid_row.addWidget(QLabel("RFIDs:"))
        # Assigned color button
        view3d_rfid_assigned_btn = QPushButton()
        view3d_rfid_assigned_btn.setFixedSize(50, 25)
        assigned_rgb = getattr(self.viewer_3d, 'rfid_color_assigned', (51, 204, 102))
        view3d_rfid_assigned_btn.setStyleSheet(f"background-color: rgb{assigned_rgb};")
        view3d_rfid_assigned_color = list(assigned_rgb)
        def change_view3d_rfid_assigned_color():
            color = QColorDialog.getColor(QColor(*assigned_rgb), dialog, "RFID Assigned Color")
            if color.isValid():
                view3d_rfid_assigned_color[0], view3d_rfid_assigned_color[1], view3d_rfid_assigned_color[2] = color.red(), color.green(), color.blue()
                view3d_rfid_assigned_btn.setStyleSheet(f"background-color: rgb({color.red()}, {color.green()}, {color.blue()});")
        view3d_rfid_assigned_btn.clicked.connect(change_view3d_rfid_assigned_color)
        view3d_rfid_row.addWidget(view3d_rfid_assigned_btn)
        # Magnet-only color button
        view3d_rfid_magnet_btn = QPushButton()
        view3d_rfid_magnet_btn.setFixedSize(50, 25)
        magnet_rgb = getattr(self.viewer_3d, 'rfid_color_magnet', (255, 204, 51))
        view3d_rfid_magnet_btn.setStyleSheet(f"background-color: rgb{magnet_rgb};")
        view3d_rfid_magnet_color = list(magnet_rgb)
        def change_view3d_rfid_magnet_color():
            color = QColorDialog.getColor(QColor(*magnet_rgb), dialog, "RFID Magnet-only Color")
            if color.isValid():
                view3d_rfid_magnet_color[0], view3d_rfid_magnet_color[1], view3d_rfid_magnet_color[2] = color.red(), color.green(), color.blue()
                view3d_rfid_magnet_btn.setStyleSheet(f"background-color: rgb({color.red()}, {color.green()}, {color.blue()});")
        view3d_rfid_magnet_btn.clicked.connect(change_view3d_rfid_magnet_color)
        view3d_rfid_row.addWidget(view3d_rfid_magnet_btn)

        # Unassigned color button
        view3d_rfid_unassigned_btn = QPushButton()
        view3d_rfid_unassigned_btn.setFixedSize(50, 25)
        unassigned_rgb = getattr(self.viewer_3d, 'rfid_color_unassigned', (230, 51, 51))
        view3d_rfid_unassigned_btn.setStyleSheet(f"background-color: rgb{unassigned_rgb};")
        view3d_rfid_unassigned_color = list(unassigned_rgb)
        def change_view3d_rfid_unassigned_color():
            color = QColorDialog.getColor(QColor(*unassigned_rgb), dialog, "RFID Unassigned Color")
            if color.isValid():
                view3d_rfid_unassigned_color[0], view3d_rfid_unassigned_color[1], view3d_rfid_unassigned_color[2] = color.red(), color.green(), color.blue()
                view3d_rfid_unassigned_btn.setStyleSheet(f"background-color: rgb({color.red()}, {color.green()}, {color.blue()});")
        view3d_rfid_unassigned_btn.clicked.connect(change_view3d_rfid_unassigned_color)
        view3d_rfid_row.addWidget(view3d_rfid_unassigned_btn)

        view3d_rfid_row.addWidget(QLabel("Cube Size:"))
        view3d_rfid_size_spin = QSpinBox()
        view3d_rfid_size_spin.setRange(1, 500)
        # default to viewer's configured size (may be float) -> int
        try:
            view3d_rfid_size_spin.setValue(int(self.viewer_3d.rfid_cube_size))
        except Exception:
            view3d_rfid_size_spin.setValue(10)
        view3d_rfid_row.addWidget(view3d_rfid_size_spin)
        view3d_rfid_row.addStretch()
        view3d_style_layout.addLayout(view3d_rfid_row)
        
        view3d_style_group.setLayout(view3d_style_layout)
        view3d_layout.addWidget(view3d_style_group)
        
        # ID Label settings group
        id_label_group = QGroupBox("3D View - ID Labels")
        id_label_layout = QVBoxLayout()
        
        # Show/Hide toggles
        show_point_ids_cb = QCheckBox("Show Point IDs")
        show_point_ids_cb.setChecked(self.viewer_3d.show_point_ids)
        id_label_layout.addWidget(show_point_ids_cb)
        
        show_line_ids_cb = QCheckBox("Show Line IDs")
        show_line_ids_cb.setChecked(self.viewer_3d.show_line_ids)
        id_label_layout.addWidget(show_line_ids_cb)
        
        show_curve_ids_cb = QCheckBox("Show Curve IDs")
        show_curve_ids_cb.setChecked(self.viewer_3d.show_curve_ids)
        id_label_layout.addWidget(show_curve_ids_cb)
        
        show_rfid_ids_cb = QCheckBox("Show RFID IDs")
        show_rfid_ids_cb.setChecked(self.viewer_3d.show_rfid_ids)
        id_label_layout.addWidget(show_rfid_ids_cb)
        
        # Font size and color
        label_style_row = QHBoxLayout()
        label_style_row.addWidget(QLabel("Font Size:"))
        label_font_size_spin = QSpinBox()
        label_font_size_spin.setRange(1, 100)
        label_font_size_spin.setValue(int(self.viewer_3d.label_font_size))
        label_style_row.addWidget(label_font_size_spin)
        
        label_style_row.addWidget(QLabel("Color:"))
        label_color_btn = QPushButton()
        label_color_btn.setFixedSize(50, 25)
        label_color_rgb = self.viewer_3d.label_color
        label_color_btn.setStyleSheet(f"background-color: rgb{label_color_rgb};")
        label_color = list(label_color_rgb)
        def change_label_color():
            color = QColorDialog.getColor(QColor(*label_color_rgb), dialog, "Label Color")
            if color.isValid():
                label_color[0], label_color[1], label_color[2] = color.red(), color.green(), color.blue()
                label_color_btn.setStyleSheet(f"background-color: rgb({color.red()}, {color.green()}, {color.blue()});")
        label_color_btn.clicked.connect(change_label_color)
        label_style_row.addWidget(label_color_btn)
        label_style_row.addStretch()
        id_label_layout.addLayout(label_style_row)
        
        id_label_group.setLayout(id_label_layout)
        view3d_layout.addWidget(id_label_group)
        ...
        def apply_settings():
            ...
            self.viewer_3d.set_render_settings(
                ...
                ,
                rfid_color_assigned=tuple(view3d_rfid_assigned_color),
                rfid_color_unassigned=tuple(view3d_rfid_unassigned_color),
                rfid_color_magnet=tuple(view3d_rfid_magnet_color),
                rfid_cube_size=float(view3d_rfid_size_spin.value()),
                show_point_ids=show_point_ids_cb.isChecked(),
                show_line_ids=show_line_ids_cb.isChecked(),
                show_curve_ids=show_curve_ids_cb.isChecked(),
                show_rfid_ids=show_rfid_ids_cb.isChecked(),
                label_font_size=float(label_font_size_spin.value()),
    ...
    def _handle_curve_click(self, pdf_x: float, pdf_y: float):
        """Handle curve creation clicks (start, end, then free center)."""
        if not hasattr(self, '_curve_creation_points'):
            self._curve_creation_points = []

        # First two clicks must correspond to existing points
        if len(self._curve_creation_points) < 2:
            point = self._find_closest_point(pdf_x, pdf_y, tolerance=10.0)
            if not point:
                self.update_status("No point found at click location")
                return

            if len(self._curve_creation_points) == 1 and point.id == self._curve_creation_points[0]:
                self.update_status("Curve end must differ from the start point")
                return

            self._curve_creation_points.append(point.id)

            if len(self._curve_creation_points) == 1:
                self.update_status(f"Curve start: point {point.id}. Click end point.")
            else:
                self.update_status(f"Curve end: point {point.id}. Click to place a point along the curve.")
            return

        # Third click uses the raw click location as a point along the curve
        start_id, end_id = self._curve_creation_points
        start_point = self.project.get_point(start_id)
        end_point = self.project.get_point(end_id)

        if not start_point or not end_point:
            self.update_status("Curve creation failed - start or end point missing")
            self._curve_creation_points.clear()
            return

        arc_z = (start_point.z + end_point.z) / 2.0
        # Create via point as hidden - it's purely theoretical for geometry calculation
        arc_point = self.operations.create_point(pdf_x, pdf_y, arc_z, description="Curve Via Point")
        arc_point.hidden = True

        curve = self.operations.create_curve(start_id, end_id, arc_point.id)
        
        # Delete the via point since it's only used for calculation
        # The actual arc points are created inside create_curve()
        self.operations.delete_point(arc_point.id, force=True, cascade=False)
        ...
        if self.show_curves:
            for curve in self.project.curves:
                if self.show_hidden or not curve.hidden:
                    ...
                    baseline = [(start.pdf_x, start.pdf_y), (end.pdf_x, end.pdf_y)]
                    sample_points = []

                    if curve.arc_points_real:
                        for real_x, real_y, _ in curve.arc_points_real[1:-1]:
                            pdf_coords = self.geometry.real_to_pdf(real_x, real_y)
                            if pdf_coords is None:
                                sample_points = []
                                break
                            sample_points.append(pdf_coords)

                    curve_polylines.append((curve.id, baseline, sample_points))
    ...
    def _find_or_create_point_at_z(self, original_point_id: int, new_z: float) -> int:
        ...
        if self.show_hidden or not new_point.hidden:
    ...
    def _autoload_project(self):
        """Auto-load the default project if it exists."""
        autoload_path = Path(r"C:\temp\Zuend\3D\3d.dig")
    ...
    def _handle_delete_key(self):
        ...
        if current_tab == 1:  # 2D Editor
            ...
            if self.editor.points_table.hasFocus():
                model = self.editor.points_table.model()
                selected = self.editor.points_table.selectionModel().selectedRows()
                if selected:
                    point_ids = []
                    for idx in selected:
                        id_val = model.data(model.index(idx.row(), 0), Qt.ItemDataRole.DisplayRole)
                        try:
                            pid = int(id_val)
                        except Exception:
                            continue
                ...
                model = self.editor.lines_table.model()
                selected = self.editor.lines_table.selectionModel().selectedRows()
                if selected:
                    line_ids = []
                    for idx in selected:
                        id_val = model.data(model.index(idx.row(), 0), Qt.ItemDataRole.DisplayRole)
                ...
                model = self.editor.curves_table.model()
                selected = self.editor.curves_table.selectionModel().selectedRows()
                if selected:
                    curve_ids = []
                    for idx in selected:
                        id_val = model.data(model.index(idx.row(), 0), Qt.ItemDataRole.DisplayRole)
