...
class GeometryEngine:
    ...
    def real_to_pdf(self, real_x: float, real_y: float) -> Optional[Tuple[float, float]]:
        """Transform real-world coordinates back to PDF space."""
        if self.transformation_matrix is None:
            return None
        ...
        real_point = np.array([real_x, real_y, 1.0])
        pdf_point = inverse_matrix @ real_point
        return float(pdf_point[self.A]), float(pdf_point[self.B])
